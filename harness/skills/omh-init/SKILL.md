...
description: |
  ...
  받아 `omc-harness`, `omc`(마켓명), `<your-github-handle>` 자리표시자를 일괄 치환하고, 초기화 마커를
  남긴 뒤 자신을 삭제한다.
...
**1회성 부트스트랩 스킬**. 템플릿 레포(`omc-harness`)를 포크한 뒤 자기 플러그인으로 만들기 위한
...
- 현재 레포가 `omc-harness` 템플릿 기반인지 확인 (루트 `.claude-plugin/marketplace.json`의
  `"name": "omc"`와 `plugins[0].name: "omc-harness"` 존재 여부로 판정)
...
> 권장: `oh-my-<org>-<domain>` 또는 `<org>-harness` — 템플릿 lineage가 섞이지 않게 `omc-harness-*` prefix는 피하세요.
...
| from | to |
|------|----|
| `omc-harness` | `{pluginName}` |
| `"name": "omc"` (마켓) | `"name": "{abbrev}"` |
| `<your-github-handle>` | `{handle}` |

`grep` 으로 실제 매칭 파일 전체 목록을 뽑아 유저에게 보여준다:

```bash
grep -rl "omc-harness\|<your-github-handle>" \
...
1. 각 대상 파일에 대해 `sed -i '' 's/omc-harness/{pluginName}/g'` (macOS 기준)
...
3. `<your-github-handle>` → `{handle}` 일괄 치환
4. 완료 마커 생성:
   ```bash
   mkdir -p .{abbrev}
...
다음 단계:
  ...
  4. git add -A && git commit -m "chore: bootstrap {pluginName} from omc-harness template"
```
...
- `omc-harness-*` prefix 확장 지원 (템플릿 lineage 혼란 원인, 비추)
